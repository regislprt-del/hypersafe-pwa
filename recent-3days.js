(() => {
  ...
  function renderRecentThreeDays() {
    ...
    body.innerHTML = arr.map(e => `<tr><td>${new Date(e.occurred_at).toLocaleString('fr-FR',{day:'2-digit',month:'2-digit',hour:'2-digit',minute:'2-digit'})}</td><td>${typeInfo(e.kind)?.[1]||e.kind}</td><td>${fmt(resultRateForEvent(e))}</td></tr>`).join('') || '<tr><td colspan="3">Aucun événement sur les 3 derniers jours</td></tr>';
  }
