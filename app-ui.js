async function toggleSlot(kind, slot) {
  const day = localDay();
  const existing = events.find(e => e.event_day === day && e.kind === kind && e.slot_no === slot);
  if (existing) {
    const { error } = await sb.from('events').delete().eq('id', existing.id);
